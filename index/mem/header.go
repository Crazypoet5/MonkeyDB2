...
const (
	...
	FREE_LIST_OFFSET    = 24 //duplicated
	FREE_OFFSET         = 24
)
...
func (mb *ManagedBlock) GetFreePos() uint {
	data, err := mb.Read(FREE_LIST_OFFSET, 8)
	if err != nil {
		log.WriteLog("err", err.Error())
	}
	return bytes2uint(data)
...
func (mb *ManagedBlock) AddFreePos(i uint) {
	p := mb.GetFreePos()
	p += i
	_, err := mb.Write(FREE_LIST_OFFSET, uint2bytes(p))
	if err != nil {
		log.WriteLog("err", err.Error())
	}
}
